...
/**
 * Virtualized version of LeadsTable for handling large datasets efficiently.
 * Uses react-window for virtualization to render only visible rows.
 */
const VirtualizedLeadsTable: React.FC<VirtualizedLeadsTableProps> = memo(({
  leads,
  loading,
  onViewLead,
  onEditLead,
  ...
  // Handle sorting
  const handleSort = useCallback((field: keyof SupabaseLead) => {
    if (sortField === field) {
  ...
  }, [leads, selectedStatus, selectedSource, searchTerm, sortField, sortDirection, inquiryTypes]);

  // Render sort icon
  const renderSortIcon = useCallback((field: keyof SupabaseLead) => {
  ...
  const SortableHeader = useCallback(({ field, label, className }: { field: keyof SupabaseLead, label: string, className?: string }) => (
    <th 
      className={cn(
  ...
  const Row = useCallback(({ index, style }: { index: number; style: React.CSSProperties }) => {
    const lead = filteredAndSortedLeads[index];
  ...
  }, [filteredAndSortedLeads, user, inquiryTypes, onViewLead, onEditLead, onDeleteLead, onStatusChange]);
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableBody>
              <List
                height={height}
                itemCount={filteredAndSortedLeads.length}
                itemSize={itemHeight}
                width="100%"
              >
                {Row}
...
VirtualizedLeadsTable.displayName = 'VirtualizedLeadsTable';
